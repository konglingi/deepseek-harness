...
| client→server | `session/cancel` | `SessionCancelParams` → `SessionCancelResult` (whether a live agent was interrupted) |
...
- **No session-close method** — `session/cancel` interrupts a turn, but an SDK-created session's agent lives until runtime shutdown; see the [`dsh-sdk-jsonrpc-server` README](../server/README.md).
