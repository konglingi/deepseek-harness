...
Coverage gaps: no test drives a real `vscode.chat` request (the chat API needs a running editor), and the participant registration, status bar, and command wiring are verified by hand in an Extension Development Host. The Python SDK does not expose `session/cancel` yet; it mirrors the protocol and can add the method when an interactive Python consumer needs it.
